...
`frontend/` 에 **Vue 프로젝트가 생성되지 않았다.** `package.json`·`src/`·`node_modules` 모두 없다.

- 아래 4번 디렉토리 구조와 8번 검증 명령어는 **스캐폴딩 이후에 유효하다.** 지금 `npm run build`를 돌리면 실패한다.
- 스캐폴딩은 전원이 공유하는 작업이다. **혼자 진행하지 말고 사용자에게 먼저 확인한다.**
- 이미 `package.json`이 있다면 이 절은 무시한다.
