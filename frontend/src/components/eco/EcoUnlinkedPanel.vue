 ...
 *
 * ── 미가입 안내를 이 화면에 둔다 ────────────────────────────────────────
 * ⚠️ **가입 여부를 알려주는 서버 값이 없다.** `POST /eco/link` 는 `X-Demo-Key` 밖에 모른다.
 * 그래서 「먼저 가입이 필요해요」라고 **단정하는 화면을 만들지 않고** 아래에 누리집으로 가는
 * 길을 상시 열어 둔다 — 서버가 아니라고 하지 않은 것을 화면이 아니라고 말하지 않는다
 * (핵심 규칙 8). 가입하고 돌아오면 위 버튼을 그대로 누르면 된다.
 */
  ...
  <div class="space-y-4">
    ...
    <GpCard tone="sub" title="아직 회원이 아니거나 등록 정보가 없나요?">
      <p class="text-body-sm text-ink-soft mt-0 mb-3">
        에코마일리지는 서울시가 운영하는 제도예요. 누리집에서 가입하고 돌아와 위 버튼을 다시
        누르면 사용량을 불러올게요.
      </p>
