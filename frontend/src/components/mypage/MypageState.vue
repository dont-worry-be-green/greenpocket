 ...
 *
 * `greenlife/GreenlifeState.vue` · `pocket/PocketState.vue` 와 거의 같지만 **일부러 복제했다.**
 * 도메인 폴더를 넘어 import 하지 않는 것이 규칙이고(frontend/AGENTS.md 4절), `ui/` 로 올리면
 * 넷이 함께 쓰는 공용 파일이 되어 스켈레톤 모양을 바꿀 때마다 다른 화면이 함께 흔들린다.
 *
 * 빈 상태에 **행동을 붙일 수 있게** 슬롯을 하나 더 뒀다 — 보관함이 비어 있을 때 안내만 남기면
 * 사용자가 다음에 무엇을 할지 알 수 없다(MY-03 은 「새 고지서 등록하기」로 이어진다).
 */
...
<template>
  <div v-if="loading" class="space-y-3 py-2" aria-label="불러오는 중">
    <div class="bg-skeleton h-28 animate-pulse rounded-xl" />
    <div class="bg-skeleton h-48 animate-pulse rounded-lg" />
  </div>
  ...
  <div v-else-if="empty" class="bg-surface rounded-lg px-5 py-10 text-center">
    <p class="text-body text-muted mt-0 mb-0">{{ emptyMessage }}</p>
