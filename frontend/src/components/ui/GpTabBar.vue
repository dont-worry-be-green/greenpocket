 ...
 *
 * 레퍼런스(design-reference/bottom_navigation.png) 실측 비율로 짰다 — 흰색 완전 라운드 pill 위에
 * 가운데 원이 위로 솟는다. 바 64 · 원 50(0.85) · 돌출 18(0.29) · 아이콘 20. 값은 전부 main.css 토큰이다.
 * 실제 화면에서는 fixed 라야 스크롤과 무관하게 붙어 있고, 하단 노치는 safe-area-inset 으로 피한다.
 * 가운데 What-if 라벨은 활성 여부와 무관하게 늘 nav-on 이다(브랜드 진입점).
 *
...
<template>
  ...
  >
    <nav class="bg-surface shadow-float flex h-(--gp-tabbar-h) rounded-full" role="tablist">
      ...
      >
        <!--
          가운데 원은 바 위로 --gp-fab-lift(18px) 솟아 있다. absolute 로 띄우는 이유는
          흐름에 두면 나머지 4개 탭의 라벨 높이까지 같이 밀려서 라벨 줄이 어긋나기 때문이다.
        -->
        <span
          v-if="t.fab"
          class="text-on-primary shadow-fab absolute -top-(--gp-fab-lift) left-1/2 z-30 flex size-(--gp-fab) -translate-x-1/2 items-center justify-center rounded-full bg-(image:--gp-grad-fab)"
        >
