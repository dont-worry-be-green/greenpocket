...
const selectedBillType = ref(route.query.type ?? 'MANAGEMENT')
const billingMonth = ref(route.query.month ?? store.targetMonth?.targetYearMonth ?? '2026-08')
...
function keepDecimal(event, utilityType) {
  const normalized = event.target.value.replace(/[^\d.]/g, '').replace(/(\..*)\./g, '$1')
  values[utilityType].usage = normalized
}
