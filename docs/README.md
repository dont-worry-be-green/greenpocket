...
| # | 파일 | 무엇 |
|---|---|---|
| 1 | `feature-spec/기능명세서.xlsx` | **기능 기준 원본** — 현재 105건. JWT 기능 COM-13~16은 다음 단계에서 동기화 예정 |
| 2 | `database/schema.sql` | **데이터 기준** — 스키마 원본. 제약 전부 포함 |
| 3 | `api/api-spec.md` | **API 계약** — 엔드포인트 64개 |
...
`feature-spec/기능명세서.md`는 검색·diff·AI 참조용 사본입니다. 원칙적으로 두 파일이 다르면 엑셀이 기준이지만, **2026-09-07 JWT 결정(COM-13~16)은 이번 문서 단계에서 Markdown에 먼저 반영했으므로 XLSX 동기화 전까지 해당 기능에 한해 Markdown과 `auth/jwt-auth.md`를 따릅니다.**
