...
- **값을 추정하거나 그럴듯한 숫자를 채워 넣지 않는다.** 1인 가구 평균 사용량은 공식 원자료와 명시된 표본·가중치·환산식으로만 만들며, 비교군의 한계를 함께 제공한다(결정 C-29·C-35).
- 1인 가구 기준선은 `resources/data/single-household-utility-baselines.json`의 검증된 공개자료를 사용한다. 전기·도시가스는 KESIS 2023년 기준 14차 마이크로데이터의 월별 가구 횡단가중 평균, 수도는 일 사용량에 조회 월 일수를 곱한 값이다.
- 비교군·출처·기준 연도·산출 방식·자료 한계를 응답에서 숨기지 않는다. 사용자 고지서가 없는 시계열 월은 `null`로 반환하며 보간하지 않는다.
