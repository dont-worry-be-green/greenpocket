...
도메인 구분은 `docs/api/api-spec.md` 15.1 「API 69개 한눈에 보기」의 영역과 1:1이다.

```
com.greenpocket
├── auth/         회원가입·로그인·토큰 재발급·로그아웃      API 4  · 4.5~4.8절
├── user/         데모 사용자, 지역 메타, 데모 초기화        API 4  · 4절
├── profile/      가입 본인정보·정책 추천 프로필             API 3  · 5절
├── bill/         관리비·전기·수도·가스 고지서, OCR (ocr/)   API 9  · 6절
├── diagnosis/    1인 가구 평균 사용량·작년 동월 비교 진단    API 3  · 7절
...
- **비교 기준선이 없어도 에러가 아니다.** `200` + `available: false` + `unavailableReason`으로 응답한다 (현재 `NO_BASELINE`).
- **값을 추정하거나 그럴듯한 숫자를 채워 넣지 않는다.** 1인 가구 평균 사용량은 공식 공개 집계와 명시된 환산식으로만 만들며, 월별 실측·전체 주거유형 평균이 아닌 값은 그 한계를 함께 제공한다(결정 C-29).
- 1인 가구 기준선은 `resources/data/single-household-utility-baselines.json`의 검증된 공개자료를 사용한다. 전기·도시가스는 연간 에너지 집계의 월평균 환산값, 수도는 일 사용량에 조회 월 일수를 곱한 값이다.
- 비교군·출처·기준 연도·환산 방식·자료 한계를 응답에서 숨기지 않는다. KESIS 월별 마이크로데이터를 확보하기 전까지 전기·도시가스를 월별 실측 평균이라고 표현하지 않는다.
